...
template <class T>
template <class X>
tree<T>::node<X>::node(node<X> *p, std::string init)
{
    parent = p;
    type=init;
    children();
...
template <class T>
template <class X>
tree<T>::node<X>::node(std::string init)
{
    parent =0;
    type=init;
    isLroot=true;
...
//init the local root on children
template <class T>
template <class X>
void tree<T>::node<X>::hasLroot()
{
    if(parent->isLroot==false)
        lRoot=parent->lRoot;
    else if(parent->isLroot==true)
        lRoot=parent;
}
...
*/
template <class T>
int tree<T>::push( T content, std::string type, bool indexed )
{
    if(!full)
    {
...
template<class T>
void tree<T>::newNode(node<T> * N, T content,std::string type)
{
...
template<class T>
bool tree<T>::moveTo(T content, std::string type)
{
    if(moveTo(type))
    {
...
template<class T>
bool tree<T>::moveTo(std::string type)
{
    std::string stopper;// this is to prevent an infinant loop when this gets called and the item is not indexed
    node<T> * tmp;
    tmp= index;
...
template<class T>
bool tree<T>::isIndex(std::string type)
{
    std::string stopper;// this is to prevent an infinant loop when this gets called and the item is not indexed
    node<T> * tmp;
    tmp= index;
