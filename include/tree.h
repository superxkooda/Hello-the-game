...
class tree
{
    ...
    struct node
    {
        ...
        node * right;
        node * parent;
        node * lRoot; //points to the local root if there is one
        bool isLroot; //is this a local root for a given leg? eg the first leg in a divergent path. a type change.
private:
        void children();
        void hasLroot();
    };
...
public:
    ...
    bool loop();//when called set current to clocal root to preform a loop through the same info. if called in multiples it will step back to the previous local root of the local root
    int push(T content, std::string type);
    int push( T content, std::string type, bool indexed );//for indexing obviously;
    T * peek();//for now its one function to retun an array of type and content;
    void done();
};
